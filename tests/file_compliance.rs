...
fn explicit_file_limits() -> HashMap<&'static str, usize> {
    ...
    // `src\tess\mod.rs` was split into focused submodules (priority_queue,
    // region, sweep, dirty_regions, connect) and now lives under the 800-line
    // default — its frozen entry is intentionally gone.
